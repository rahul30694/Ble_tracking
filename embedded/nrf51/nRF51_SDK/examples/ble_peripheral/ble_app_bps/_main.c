...
#define DEVICE_NAME                     "IOHertz"                               /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME               "IOHertz"                      /**< Manufacturer. Will be passed to Device Information Service. */
#define MODEL_NUM                       "IOHertz"                           /**< Model number. Will be passed to Device Information Service. */
...
#define LOGLINE(s, ...)      logline(__FILE__, __func__, s, ##__VA_ARGS__)

char *strlchr(const char *str, char c)
{
    char *p = (char *)str;
    char *t = p;
    while ((t = strchr(t, c)) != NULL) {
        p = t;
        t++;
    }
    return p;
...
void logline(const char *file, const char *func, const char *str, ...)
{
    va_list l;
    va_start(l,str);
    char *_file = strlchr(file, '/');
    _file++;
    printf("%s: %s: ", _file, func);
    vprintf(str, l);
    printf("\r\n");
    va_end(l);
}
...
static void on_bps_evt(ble_bps_t * p_bps, ble_bps_evt_t * p_evt)
{
    switch (p_evt->evt_type)
    {
        ...
        case BLE_BPS_EVT_INDICATION_DISABLED:
            LOGLINE("BLE_BPS_EVT_INDICATION_DISABLED");
            break;
        default:
...
static void services_init(void)
{
    ...
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&bps_init.bps_meas_attr_md.read_perm);
    ...
    switch(p_event->event_id)
    {
        case DM_EVT_LINK_SECURED:
            ...
            if (is_indication_enabled) {
            	LOGLINE("Sending Data as Link is Encrypted");
                blood_pressure_measurement_send();
            }
            break;

        case DM_EVT_CONNECTION:
        	LOGLINE("DM_EVT_CONNECTION");
			dm_security_status_t status;

        	dm_security_status_req(p_handle, &status);

        	if (status == NOT_ENCRYPTED) {
        		LOGLINE("NOT_ENCRYPTED");
        		//if (dm_security_setup_req((dm_handle_t *)p_handle) != NRF_SUCCESS)
        			//LOGLINE("FAILED");
        		err_code = sd_ble_gap_disconnect(m_conn_handle, BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
                //p_event->event_id = DM_EVT_DISCONNECTION;
        		//APP_ERROR_CHECK(err_code);
        	} else if (status == ENCRYPTION_IN_PROGRESS) {
        		LOGLINE("ENCRYPTION_IN_PROGRESS");
        	} else {
        		LOGLINE("ENCRYPTED");
        	}
        	break;
        case DM_EVT_DISCONNECTION:
        	LOGLINE("DM_EVT_DISCONNECTION");
        	break;
        case DM_EVT_SECURITY_SETUP:
        	LOGLINE("DM_EVT_SECURITY_SETUP");
        	break;
        case DM_EVT_SECURITY_SETUP_COMPLETE:
        	LOGLINE("DM_EVT_SECURITY_SETUP_COMPLETE");
        	break;
        case DM_EVT_SECURITY_SETUP_REFRESH:
        	LOGLINE("DM_EVT_SECURITY_SETUP_REFRESH");
        	break;
        default:
            // No implementation needed.
            break;
