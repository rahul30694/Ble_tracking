...
static uint8_t adv_manuf_data[BLE_GAP_ADV_MAX_SIZE] = "AdvManuData";
static uint8_t scan_resp_manuf_data[BLE_GAP_ADV_MAX_SIZE] = "ScanRespAdvManuData";
...
int gap_params_init(void)
{
    ...
    uint8_t add[6] = { 0x9A, 0x01, 0x02, 0x03, 0x04, 0xC0 };
...
int advertising_init(void)
{
    ...
    m_manuf_adv.data.size = strlen(adv_manuf_data);
    ...
    m_manuf_scan_resp.data.size = strlen(scan_resp_manuf_data);
...
{
    printf("RX Length - %u\r\n", length);
    if (ble_trs_transmit(p_data, length))
